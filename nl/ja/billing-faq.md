...
削除されたアカウント内のデータを復旧することはできません。

## サポートを依頼するには、どのようにすればよいですか
{: #contactsupport}
{: faq}

コンソール・メニュー・バーの**「サポート」**をクリックして、サポート・センターにアクセスします。 そこで、まずは、よくある FAQ のリストを活用してください。 必要な回答が見つからない場合は、**「お困りですか?」**セクションを参照して、IBM Cloud サポートにお問い合わせください。
...
ライト・アカウントでは、アプリに最大 256 MB の瞬間的ランタイム・メモリーがあります。 ライト・アカウントで使用可能な無料枠を使い尽くすまでアプリをデプロイできます。 メモリーを増やす必要がある場合は、有料アカウントにアップグレードすると、512 MB の無料の瞬間的ランタイム・メモリーを取得でき、その限度を超えて使用した分のみが支払いの対象となります。 有料アカウントでは、作成可能なアプリの数は無制限です。
...
サブスクリプションは、お客様が特定の利用期間と支出額で {{site.data.keyword.Bluemix_notm}} を使用することを確約する、お客様とIBM との間の契約です。 利用期間が終わる前にサブスクリプションの取り消しを要求することはできますが、サブスクリプションを取り消すことができるかどうかは IBM の裁量によります。 サブスクリプションの残りのクレジットは、失効することがあります。 この可能性について照会するには、サポートにお問い合わせください。 サブスクリプションを取り消さなければならない理由について、詳しく説明するようにしてください。
...
## 消費量が特定のレベルに到達したときに、通知を受け取ることはできますか?
{: #spending-notify}
{: faq}

アカウント、コンテナー、ランタイム、すべてのサービス、および特定のサービスごとに別個の消費量しきい値を設定することができます。 1 カ月の消費量がそれらのしきい値の 80%、90%、および 100% に到達すると、自動的に通知を受け取ります。 消費量の通知を設定するには、**「管理」>「請求および使用量」**をクリックし、**「消費量の通知」**を選択します。 詳しくは、[消費量通知の設定](/docs/billing-usage?topic=billing-usage-spending)を参照してください。
